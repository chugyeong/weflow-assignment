...
const LandingBenefits = () => {
  return (
    <section className="mt-24 max-md:mt-10">
      <div className="mb-[34px] max-w-[820px] max-[640px]:mb-6">
      ...
      <ul className="grid grid-cols-5 gap-4 max-[1100px]:grid-cols-2 max-[560px]:grid-cols-1 max-[640px]:gap-3.5">
        {LANDING_BENEFITS.map((benefit) => {
          const Icon = benefit.icon;

          return (
            <li key={benefit.title} className={cardClass}>
              <div className="mb-[18px] flex h-[42px] w-[42px] items-center justify-center rounded-[10px] bg-we-blue-500 text-we-white max-[640px]:mb-3 max-[640px]:h-8 max-[640px]:w-8 max-[640px]:rounded-[8px]">
                <Icon className="h-5 w-5 max-[640px]:h-4 max-[640px]:w-4" />
              </div>
              <h3 className="text-xl text-we-white max-[640px]:text-base">{benefit.title}</h3>
              <p className="mt-2 leading-[1.7] text-we-gray-500 max-[640px]:text-[13px] max-[640px]:leading-[1.55]">{benefit.description}</p>
            </li>
          );
        })}
      </ul>
